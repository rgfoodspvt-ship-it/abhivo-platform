...
export const getPolygons = (village: string, district?: string) => fetchAPI<any>(`/map/polygons?village=${encodeURIComponent(village)}${district ? '&district=' + encodeURIComponent(district) : ''}`);
